...
这是斗地主项目的全新 TypeScript 重构版本。新项目不兼容旧 Vue 2 客户端，目标是服务端权威、协议强类型、规则纯函数、前端 Phaser 表现层。
...
- `@ddz/game-server`：Colyseus 房间，支持 JWT 入房、按房间号隔离牌桌、房间状态同步、服务端权威回合倒计时、超时自动不叫/不抢/过牌/出牌、可配置机器人补位、基于手牌牌力的机器人叫地主/抢地主、带公开快照的对局事件写入、断线重连、开局前离房释放座位、准备、叫地主、抢地主、出牌、过牌、结算和显式拒绝非法命令。
